...
export function validateCaptureCommand(
  ...
  if (typeof input.idempotencyKey !== "string") {
    throw new CaptureValidationError("INVALID_IDEMPOTENCY_KEY", "缺少有效的幂等键。");
  }
  ...
  return {
    // Preserve the exact user-authored string. Never trim or rewrite it.
    rawContent: input.rawContent,
    occurredAt: new Date(parsedOccurredAt).toISOString(),
    idempotencyKey,
  };
}
